...
def refresh_token(refresh_token: RefreshToken):
    ...
    """
    refresh_token_val = refresh_token.refresh_token
    # Validate the refresh token and get the user
    if not (payload := verify_refresh_token(refresh_token_val)):
        raise HTTPException(status_code=400, detail="Not authorized")

    username = payload.get("sub")

    # Check if the refresh token is revoked
    if RevokedToken.is_revoked(refresh_token_val):
...
def logout(refresh_token: RefreshToken):
    ...
    refresh_token_val = refresh_token.refresh_token
    # Validate token
    if payload := verify_refresh_token(refresh_token_val):
        # Extract payload
